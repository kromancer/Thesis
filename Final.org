...
** Problem Definition
The analytic description of the SystemC's simulation environment, presented in Section [[SystemC's Discrete Event Simulator]], yields a categorical verdict: if parallel simulation is to be achieved, a new simulation environment must be built, from the ground up.
...
** Hypothesis
The \textit{primum movens} of this project can be summarized as follows:
there is a healthy amount of parallelism available in the simulation of Electronic Systems, especially in the context of virtual platforms, where hardware and software are co-simulated.
...
Processes introduce a spatial decomposition of a system;
...
*Causality*, as a philosophical assumption about the behaviour of a system, has been mathematically captured by the properties we imposed on communication and execution.


*** Causality and Concurrency
The binary relationship *"causally affects"*, denoted by the symbol $\propto$, is introduced.
...
It is this absence of a SystemC TLM 2.0 compatible OoO PDES framework that justifies any novel approach on the matter.
...
In a detailed hardware simulation, communication is realized through a number of pin level events.
which triger context switches inside the simulator, 
