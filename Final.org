...
Section [[The Discrete Event Model of Computation]] hopes to help the reader understand why *Electronic System-Level Design Language* (ESLDL) models can be simulated.
It concludes by presenting an algorithm describing SystemC's simulation engine.
...
In Section [[Sequential Discrete Event Simulation]], SystemC's simulation engine is presented as an algorithm that realizes the operational semantics of a sequential *Discrete Event Simulator* (DES).
...
The time axis is not resolved; a *"clock"* mechanism for relating an event with a number, its *timestamp*, has not been defined.
...
The definition of clocks is deferred to the implementation of the DE abstract machine.
...
Since $\mathds{E}$ is partially ordered and only the sets $P_i$ are totally ordered, we are forced to reach the conclusion that the DE MoC instigates a *relativistic notion of time*.
...
SystemC's reference implementation of the DES, referred to as the *SystemC kernel* \cite{OpenSystemCInitiative2012}, is an example of a sequential realization of the DE MoC.
It is a sequential implementation because spatial decomposition is emulated through the co-routine mechanism (also known as co-operative multitasking). 
Processes perform their actions (computation, communication), henceforth *run*, without interruption.
In a single core machine that means that only a single process can be running at any moment in real time.
No other process can run until the running process has yielded.
A non-running process shall not preempt or interrupt the running process \cite{OpenSystemCInitiative2012}.

A sequential implementation demands the presence of a simulation director, henceforth *kernel*.
It is the kernel's responsibility to create a total ordering of simulation events, to switch "focus" between processes and to facilitate process communication.




To perform his duties, the kernel maintains a *centralized event list* sorted by timestamps and knows which process is *running*, which are *runnable*, and which processes are waiting for events.
...
This means that there is a notion of two actions (communication or computation) being either ordered in time
(one occurs before the other) or being simultaneous.

Only the top-level director advances time.
We all harbor a naive notion of a uniform fabric of time, shared by all participants in the physical world.
By such a notion is a fiction, and real systems are strongly affected by errors in time measurement and discrepancies
between logical and physical notions of time.

Each event has both a value and a timestamp, where the timestamp defines a global ordering between events.
A timestamp is a superdense time value, consisting of a model time and a microstep.
A DE model executes chronologically, processing the oldest events (those with earlier time stamps) first.
Time advances as events are processed, both in the model and in the outside world.
There is potential confusion, therefore, between model time, 
which is the time that evolves in the model, and real time, 
which is the time that elapses in the real world while the model executes.
...
+ In Section [[The Discrete Event Model of Computation]] a different approach is adopted for presenting the DE MoC, 
  ...
  Time modeling is susceptible to different realizations depending on whether the DE MoC will be executed on a sequential or parallel machine.
  Lamport's "happens before" relationship \cite{Lamport1978} suffices in revealing the timed and concurrent nature of the MoC.
