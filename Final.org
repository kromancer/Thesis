#+TITLE: SPELLCHECK Parallel Simulation of SystemC Loosely-Timed Transaction Level Models
...
** TODO Objectives
Parallel Discrete Event Simulator proposal
In order to reach The answer of requires the development of the following software applications:

+ A DE MoC with concepts distributed computing. Maybe mention lamport's award announcement.
...
** TODO Hypothesis
The \texttt{primum movens}
healthy amount of parallelism available in the simulation of Electronic Systems
...
+ *Transferability*: The verification of two case studies can only be the basis step of inductive inference.
                     There is still the induction step, that is hoped to be adressed by the proposition of a compiler, that will allow every Loosely-Timed Transaction Level Model to "fit" the proposed Parallel Discrete Event Simulator.
...
Causality, as a philosophical assumption about the behaviour of the system, can now be interpreted by the following statement: 

#+BEGIN_EXPORT latex
\begin{equation}
e_1,e_2 \in \mathds{E} .  e_1 \propto e_2 \implies e_1 \sqsubset e_2
\end{equation}
#+END_EXPORT
...
Logic time modeling is deferred to the implementation of the DE abstract machine and is highly depended on the nature of the undelying hardware. 
Is it *parallel*, where the spatial decomposition defined in the DE can be preserved? 
Or is it *sequential*, where space dimensionality must be emulated.
The only restrictions DE semantics impose on a logic time modelling technique $C$ are:
...
+ \texttt{notify(sc\_time t)}:     (Scheduled occurence) The process adds the event to the global event queue. All sensitive processes will become runnable when the kernel triggers the event.
...
+ \texttt{notify(SC\_ZERO\_TIME)}: (Delayed occurence)   The process adds the event to delta notification queue. All sensitive processes in the waiting list are moved to the runnable list, after the runnable list becomes empty.
...
+ \texttt{wait(sc\_event e)}  Before yielding, the process modifies its sensitity list, so as to include \texttt{e}
...
In unit [[Prior Art]] we give an overview of prior art in the field of PDES in ESLD.
is an event safe to execute
...
Please note that the term process in this section refers to the Operating System concept.

*** Rationale
*Message Passing Interface* 3.0 (MPI) was the preferred implementation framework for the proposed OoO PDES.
Events are modeled as structured messages, and communication as message passing.
...
The goal of this chapter is to present the process synchronization algorithm that will be applied in the proposed OoO PDES.
...
The system's state, a set of variables, is partioned amongst the system's processes.
...
The intuition behind any technique, that could break the deadlock, should rely on the followig observation:
if CDG knew that SKG is at time 5, then it could be able to accept the incoming fligh from ARL, without breaking causality.
...
\begin{algorithmic}[2]
   ...
   \While{process clock < some T}  
      ...
      \State post a MPI\_Issend to each outgoing link L with timestamp = counter + Lookahead(myRank, recvRank)
   \EndWhile
...
For example, the 3 airport topology described in Figure [[fig:deadlock]] is demonstrated in Appendix \ref{AppendixC}
Every airport is given a schedule of departures and the system's computational objective is to create a log of departures/arrivals per airport.

An airport's main event loop is demonstrated in Figure \ref{fig:airport}.
The series of communication primitives presented in Algorithm \ref{CMB_mpi} have almost the same aggregate effect as the collective communication primitive \texttt{MPI\_Neighbor\_Allgather}.
...
\begin{tikzpicture} [
  ...
  \matrix [column sep=5mm, row sep=5mm] {
                    ...
                    & \node [block, text centered, text width=5cm] (n2) {\small\texttt{Event e = links.nextEvent()} \small\texttt{counter = e.tstamp()}};    & \\
  ...
  \end{scope}
\end{tikzpicture}
\caption{Case Study 1: Airport's event loop (in \texttt{Process::run()})} 
\label{fig:airport}
\end{figure}
#+END_EXPORT
...
there is a need for an \texttt{sc\_clock} instance, which is nothing more than a predifined process, functioning in the same way as the "time" process.
...
+ *Topology mapping* To the best of our knowledge
  One of the major features of MPI's topology interface is that it can easily be used to adapt the MPI proces layout to the underlying network and system topology.
...
#+BEGIN_EXPORT latex
\subsection{SystemC: Non-Determinic yet Repeatable}
\label{AppendixB}
#+END_EXPORT
...
** MPI Example: The Pipeline Pattern :ignore:
#+BEGIN_EXPORT latex
...
Just think of how cars are manufuctured.
