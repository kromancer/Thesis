...
but will conform to the general context dictated by the documents that have been already signed (documents' titles:  "Statement of Terms and Conditions of Fixed Term Employment" and "Employee Agreement") .
...
we will yield equivalent and faster simulations with respect to the reference SystemC DES.   
...
1. A OoO PDES implementation of the SystemC kernel.
2. A static analysis/introspection tool for parsing the SystemC description of the system and extracting its pure representation, in terms of processes and links.
3. A code generation tool for implementing the communication FIFOs and replacing .
4. A way of sequencing the application of the previous tools, either in the kernel's elaboration phase, or using a "gluing" script.
5. A TLM 2.0 coding style to minimize the effort and complexity of the analysis and generation tools.
6. A proof of concept application of the proposed methodology on a sufficiently parallel system, running a substantially parallel application, on top of a Linux kernel.
...
By assuming/enforcing the principle of one process per module, we hope to cope with causality errors caused by processes sharing system variables.

Furthermore, the feasibility of the introspection and code generation procedures certain limitations on SystemC's expressive capabilities will be imposed.
...
Using SystemC's well defined API for module hierarchy (for example \texttt{get_child_objects()}) Not in a pre-compilation, application's runtime, at elaboration phase () by querrying each object, using 
...
Xeon phi coprocessor or Xeon server
Stretch goal to investigate the distribute the simulation , a cluster of SMP nodes.

** Evaluation Metrics
The fundamental metric for evaluating the efficiency of the PDES implementation proposed is the speedup compared to the reference DES.
Scaling
...
#+LATEX_HEADER: \chead{Project Plan Draft 2}
