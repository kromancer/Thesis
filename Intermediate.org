...
DO NOT FORGET TO SPELLCHECK
DO NOT FORGET TO INSERT THE IMAGES
DO NOT FORGET TO CHECK LINKS
...
In unit [[MPI Realization of CMB]] we provide pseudocode for the realization of the CMB using the MPI communication primitives.
...
\begin{algorithmic}[2]
   ...
   \While{process clock < some T}  
      ...
      \State set clock = time-stamp(M)
      \State process event M
      \State send either a null or meaningfull event to each outgoing link L with time-stamp = clock + Lookahead(clock,L,...)
   \EndWhile
...
MPI is a message passing library interface specification, standardized and maintained by the Message Passing Interface Forum \cite{citation_needed}.
It is currently available for C/C++, Fortran and Java from multiple vendors (Intel, IBM, OpenMPI) \cite{citation_needed}.
...
The basic communication primitives are the functions \texttt{MPI_Send(...)} and \texttt{MPI_Recv(...)}.
...
By default, message reception is blocking, while message transmition may or may not block.
...
and both match the same receive (a call to \texttt{MPI_Recv}), 
...
The MPI API contains a number of variants, or modes, for the basic communiction primitives.
They are distinguished by a single letter prefix (e.g. \texttt{MPI_Isend(...)}, \texttt{MPI_Irecv(...)}).
As dictated by the MPI version 3.0, the following communication modes are supported \cite{MessagePassingInterfaceForum2012}:

*No-prefix for standard mode: \texttt{MPI_Send(...)}*
...
In this case, the send call will not complete, blocking the transimiting process, until a matching receive has been posted, and the data has been moved to the receiver.

*B for buffered mode: \texttt{MPI_Bsend(...)}* 
...
*S for synchronous mode: \texttt{MPI_Ssend(...)}*
...
*R for ready mode: \texttt{MPI_Rsend(...)}*
...
*I for non-blocking mode: \texttt{MPI_Isend(...)}, \texttt{MPI_Ibsend(...)}, \texttt{MPI_Issend(...)} and \texttt{MPI_Irecv(...)*
Non-blocking message passing calls return control immediately (hence the prefix I), 
but it is the user's responsibillity to ensure that communication is complete, 
...
Listing \ref{alg:CMB_mpi} is a pseudocode, sketching out the CMB process event loop, using MPI's communication primitives.
#+BEGIN_LATEX
\begin{algorithm}
...
\begin{algorithmic}[2]
   ...
   \While{process clock < some T}  
      \State post a \textbf{MPI_Irecv} on each incoming peer process
      \State post a \textbf{MPI_Wait}: block until every receive has been completed
      \State save each message received in a seperate, per incoming link, FIFO.
      \State identify message M with the smallest time-stamp
      \State set clock = time-stamp(M)
      \State process message M
      \State post a \textbf{MPI_Issend} to each outgoing link L with time-stamp = clock + Lookahead(clock,L,...)
   \EndWhile
...
It will be measured by experimentations on the project's use case.
...
_COMMENT:_ This section will become more concrete when we start experimentations.
...
From the initiator's perspective, they act as targets and from the target's perpective they act as initiators.
...
It is a structure that contains all the necesseray information about the transaction.
...
This is a quite problematic section. You need to elaborate more, do not forget LT is on your thesis title. 
...
* Use Case
In this chapter we describe the transaction level model we are going to use for conducting our experimentations.
The purpose of the experimentations is twofold;
...
The platform is a shared memory, cache-coherent, symmetric multiprocessor system based on the OpenRisc 1200 Instruction Set Simulator.
...
We have the baremetal (newlib based) toolchain for compiling applications for the OpenRisc ISS.
