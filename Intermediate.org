...
*Keywords:* Parallel discrete event simulation, Conservative synchronization algorithms, Transaction level models, SystemC TLM
...
the IoT revolution has promised to deliver,
...
One early *stage* in the design of such systems is the in tandem simulation of hardware and software.
Transaction Level Models, expressed in the SystemC TLM 2.0  modeling language, can facilitate this co-simulation.
However, the sequential nature of the SystemC's Discrete Event simulation engine is a limiting factor.
...
Avoiding causality errors is the major obstacle one faces when trying to parallelize the simulation.
Transaction Level Modeling increasing complexity -> raise the abstraction layer of -> thus TLM was born -> quite new field that is why no one has solved it.


_4. How did you solve the problem?_
Reducing the causality problem to a synchronization problem,
and applying prior art in the field of ....
we have achieved a that uses a distributed notion of simulation time thus does not require centralized simulation moderation.
...
to allow this automatic translation the features of TLM 2.0  supported by the framework is quite limited, Loosely Timed
