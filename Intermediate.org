...
* Introduction
The aim of this chapter is to present the problem statement,
after a brief introduction of the relevant context.

In units [[The Design Process]] and [[Electronic System-Level Design]] we make an attempt to 
deobfuscate the meaning of heavily cluttered terms such as design, system, model and simulation, 
in order to provide an overview of the engineering practise of Electronic System-Level Design (ESLD).

The third and fourth unit 

In the fourth and the fifth unit the problem statement and the objectives of this Master's Thesis are presented.

The structure of this document is presented in the final unit.
...
Conceptually, designing implies a non-monotonic movement from abstract to concrete, fueled by the engineer's *design decisions*, incrementally adding implementation details.
...
A *model* is thus an abstract representation of the final artifact with abstraction being the key mental ability for managing complexity.
The design flow can be now semi-formally defined as a process of model refinement and the ultimate model being the final artifact itself.
We use the term semi-formal to describe the process of model refinement, because to the best of our knowledge, 
such model algebras that would establish transformation rules and equivalence relations are far from complete \cite{Gajski2009}.
...
An electronic system provides a desired functionallity, by manipulating the flow of electrons.
...
The prominent way for visualizing the design space and further refining the terms presented in the previous unit,
in the context of electronic systems, 
is by means of the Y-Chart.
The concept was first presented in 1983 \cite{miaou} and has been constantly evolving to match industry practices.
Figure \ref{fig:Y-Chart} presents the form of the Y-Chart found in \cite{Gajski2009}.

The Y-Chart quantizes the abstraction space into four levels; system, processor, logic and circuit, represented as the four concentric circles.
Points in the abstraction space correspond to system models.
For each abstraction level, one can have different views over the system; behavioral, structural and physical

Physical dimensionality added by automated place and route software.
geometrical positioning of the components on the silicion waffer


*Synthesis*:
The movement from behavioral to structural (arrow AB) and the 
realization of an upper level structural model  using finer components (arrows BC, BD).

Design flow vs synthesis re-itaration not , monotonic, linear progress through the design space
...
a TLM is used to explore different specification model structures,
and different mappings to platform architectures. 

For example, we may want to restructure a SM for a given platform and a given mapping. 
On the other hand, for any given SM and component library, we may try to generate an optimal platform for some given metric. 
Similarly, for a given SM and platform, we may try to find an optimal mapping for a given metric. 
In an extreme case, we may want to simultaneously optimize the SM structure or application code, platform architecture, and the mapping for one or several metrics. 
As mentioned earlier, TLMs allow us to perform quickly and with reasonable accuracy each of these disparate optimization scenarios, which will be described in more detail in Chapter 4.
...
A system level model offers an abstract description of a system, beyond the Register Transfer Level (RTL) a HDL, like VHDL or Verilog, can facilitate.
*Transaction Level Modeling* (TLM) is a term coined to describe a post-RTL abstraction space, where the system's components communicate via transactions.
