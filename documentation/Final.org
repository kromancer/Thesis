...
With operational semantics it is implied that a language can not determine computation by itself \cite{JanSan2005}. 
...
The term discrete means that the set $\mathds{A}$ of all possible system states can be enumerated by natural numbers ($|\mathds{A}| = \aleph_0$).
...
between any two events $e_1, e_2$, with modeled real time values $t_1, t_2$, $\exists e_3$, such that $timestamp(e_1) < timestamp(e_3) < timestamp(e_2)$.
...
The purpose of the timing annotation argument is to notify components that a particular transaction should occur at \texttt{sc\_time\_stamp() + delay}.
The argument \texttt{delay} is the timing annotation argument, while \texttt{sc\_time\_stamp()} is a SystemC function that returns the current simulation time.
...
In the example above, the time quantum is assumed to have been set at 1ms. The initiator keeps incrementing the timing annotation argument pass to \texttt{b\_transport} until it reaches 1ms, 
at which point the initiator calls wait and resets the timing annotation.
...
A large gives the best speedup, but the reduction in simulation accuracy may be unacceptable.

The global quantum is the time between successive sync points.
*** The Approximately-Timed coding style
An approximately-timed component should respect timing annotations and schedule them for execution at \texttt{sc\_time\_stamp() + delay}.
...
#+NAME: fig:case2
#+RESULTS:
[[file:Figures/multiprocessor.pdf]]
...
#+BEGIN_SRC cpp :tangle ../src/examples/tlm_lt_example.cpp :main no
#include "systemc"
#include "tlm.h"
...
The initiator respects the timing annotation, and synchronizes with simulation time, upon completion of every transation.
#+BEGIN_SRC cpp :tangle ../src/examples/tlm_lt_example.cpp :main no
...
\clearpage
** SystemC TLM 2.0 Example: Temporal Decoupling using a Quantum Keeper :ignore:
#+BEGIN_EXPORT latex
\subsection{SystemC TLM 2.0 Example: Temporal Decoupling using a Quantum Keeper}
\label{AppendixEB}
#+END_EXPORT
What follows is an example of an LT TLM model comprising of two *Initiators* and a *Target* component.
...
For all initiators we define the same time quantum value.
#+BEGIN_SRC cpp :tangle ../src/examples/tlm_lt_with_quantum_keeper_example.cpp :main no
#include "systemc"
#include "tlm.h"
...
The \texttt{tlm\_utils::tlm\_quantumkeeper} method \texttt{set\_and\_sync} will update the initator's local perception of time, and
...
#+BEGIN_SRC cpp :exports none :tangle ../src/examples/tlm_lt_with_quantum_keeper_example.cpp :main no
    // TLM-2 backward non-blocking transport method
    virtual tlm::tlm_sync_enum nb_transport_bw( tlm::tlm_generic_payload& trans,
...
The rest of the Initator's implementation as well as the Target component remain the same as in Appendix \ref{AppendixE}.
#+BEGIN_SRC cpp :tangle ../src/examples/tlm_lt_with_quantum_keeper_example.cpp :main no
int sc_main( int argc, char* argv[])
{
