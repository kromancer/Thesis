...
void Process::run()
{
    ...
    // The airport-process is ready to terminate when:
    ...
    bool no_inbound_flights_in_links=true;
	...
	do {
		...
		airport_log.push_back(currentEvent);
		airport_log_file << sendbuf.timestamp <<  "," << sendbuf.id << "," << sendbuf.source << "," << sendbuf.destination << endl;
		//airport_log_file <<"@" << sendbuf.timestamp <<  "\tTo: " << airport_to_string[sendbuf.destination] << "\t" << "ID: " << sendbuf.id << endl;
	    }
	    // Inbound, Irrelevant, Null or ReadyToTerminate 
	    else 
	    {
		// Inbound Flight
		if (currentEvent.id > 0 && currentEvent.destination == rank)
		{
		    airport_log.push_back(currentEvent);
		    airport_log_file << currentEvent.timestamp <<  "," << currentEvent.id << "," << currentEvent.source << "," << currentEvent.destination << endl;
		    // Knowledge needed for proper simulation termination
